\documentclass[12pt,oneside]{scrartcl}
\usepackage[utf8]{inputenc}
\usepackage[english]{babel}
\usepackage[T1]{fontenc}
\usepackage{graphicx}
\usepackage{blindtext}
\usepackage{hyperref}
\usepackage{caption}
\usepackage{float}
\usepackage{enumerate}
\graphicspath{{./resources/images/}}
\usepackage{fancyhdr}
\usepackage{lipsum}
\usepackage{geometry}
\usepackage{amsmath}
\usepackage{tabularx}
\usepackage{booktabs}
\usepackage{./resources/imports/uml/tikz-uml}
\usepackage{pgfgantt}
\usepackage{lscape}
\usepackage{./resources/imports/cryptocode/cryptocode}
\usepackage[most]{tcolorbox}

\tikzumlset{fill usecase=white}

\newcommand{\subsubsubsection}[1]{\paragraph{#1}\mbox{}\\}
\setcounter{secnumdepth}{4}
\setcounter{tocdepth}{4}
\begin{document}
\renewcommand{\figurename}{Abb.}
\pagestyle{empty}
\begin{titlepage}
    \begin{center}
        \vspace*{1cm}
            
        \Huge
        \textbf{Freenet as a broker for "Medical" IoT Data}
            
        \vspace{1cm}
        \LARGE
        by
            
        \vspace{1cm}
            
        \textbf{Yannick Stebler}
            
        \vfill
            
        A thesis presented for the degree of\\
        Bachelor of Information Science
            
        \vspace{0.8cm}
            
        \includegraphics[width=0.4\textwidth]{BFHLOGO}
            
        \Large
        Technology and Computer science\\
        BFH - Bern University of Applied Sciences\\
        Switzerland\\
        {\large \today\par}
            
    \end{center}
\end{titlepage}
\newpage
\pagenumbering{arabic}
\pagestyle{fancy}
\fancyhf{}
\rhead{Yannick Stebler}
\lhead{Freenet as a broker for "Medical" IoT Data}
\lfoot{Bern University of Applied Sciences}
\rfoot{Page \thepage}
    \include{chapters/00/00-Abstract}
    \newpage
\tableofcontents
\newpage
    \include{chapters/10/11-Intro}
    \newpage
    \include{chapters/10/12-GoalDefinitions}
    \newpage
    \include{chapters/20-planing/20-timeplan}
    \newpage
    \include{chapters/30-maincontent/30-architecture}
    \newpage
    \include{chapters/30-maincontent/31-freenet}
    \newpage
    \include{chapters/30-maincontent/39-attacks}
\end{document}

https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-56Ar3.pdf

https://cryptobook.nakov.com/asymmetric-key-ciphers/ecdh-key-exchange

https://github.com/kmackay/micro-ecc
https://github.com/kmackay/micro-ecc/blob/master/uECC.h

https://tools.ietf.org/html/rfc5869